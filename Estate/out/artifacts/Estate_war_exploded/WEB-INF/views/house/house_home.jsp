...
<body>
	...
	<div class="pageheader">
		<h2><i class="glyphicon glyphicon-user"></i>户型管理<span>所有户型</span></h2>
 ...
 <label class="control-label col-sm-1" for="txt_search_departmentname"  >选择楼盘</label>
 <div class="col-sm-3">
 ...
 <div id="toolbar" class="btn-group">
	 ...
	 <button id="btn_find" type="button" class="btn btn-default">
       ...
       <table class="table table-hover" id="cusTable"  
		       ...
		       <thead>  
		          <tr>
		              ...
		              <th data-field="logo" data-formatter="logoFormatter" >户型图片</th>
					  ...
					  <th data-field="status" data-formatter="logoFormatter2">户型状态</th>
...
function initTable() {  
    ...
    $("#cusTable").bootstrapTable({  
        ...
        url: basePath+"/house/list", //获取数据的Servlet地址  
        striped: true,  //表格显示条纹  
  ...
  $(function(){
		...
		$("#btn_add").click(function(){
			window.location.href=basePath+"/house/add?agency_id="+$("#asd").val();
		})
		$("#btn_edit").click(function(){
			var data=$("#cusTable").bootstrapTable("getSelections");
			if(data.length==1){
				window.location.href=basePath+"/house/update?id="+data[0].id+"&agency_id="+$("#asd").val();
			}else{
				swal("请选择户型","","warning");
			}
		...
		$("#btn_find").click(function(){
			var data=$("#cusTable").bootstrapTable("getSelections");
			if(data.length==1){
				window.location.href=basePath+"/house/lookHouse?id="+data[0].id+"&agency_id="+$("#asd").val();
			}else{
				swal("请选择户型","","warning");
			}
		...
		$('#btn_query').click(function(){
			$('#name').focus();
			$('#status').focus();
			$("div1").parent().next("div").text("请输入户型名称");
	    	initTable();
