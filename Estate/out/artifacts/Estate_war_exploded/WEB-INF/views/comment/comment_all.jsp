    ...
    <body>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-5 col-sm-6 hidden-xs">
                            <div class="search-box">
                                <form action="<%=path%>/user/search">
                                    <input class="form-control search-form" type="text" placeholder="点击这里搜索楼盘" />
                                    <input type="hidden" id="user_id" name="user.id" value="${user.id }">
                                    <input type="hidden" id="buildings_id" value="${buildings_id }">
            ...
            <div class="container">
            	<div id="toolbar" class="btn-group">
				 <button id="btn_look" type="button" class="btn btn-default">
				     <span class="glyphicon glyphicon-plus" aria-hidden="true"></span>我也来评价
				 </button>
				 <a data-toggle="modal" data-target="#myModal" style="display: none" id="asda"></a>
				</div>  
                <div class="row">
				    <div class="col-md-12">
				        <table class="table table-hover" id="cusTable"  
						       ...
						       data-showColumns="true">  
						       	  <colgroup>
						       		...
						       		<col style="width:300px;height:50px">
						       		<col style="width:100px;height:50px">
						       </colgroup>
						     	<thead> 
						          <tr>
							       	  <th data-field="user.headicon"  data-valign="middle" data-formatter="logoFormatter">头像</th>            
						       		  <th data-field="content"  data-valign="middle">评价内容</th>  
						       		  <th data-field="comment_time" data-valign="middle" >评价时间</th>  
						          </tr>  
						       </thead>  
						       <tbody>  
						       </tbody>  
            ...
            <div class="modal-dialog">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModalLabel">发表点评</h4>
                    </div>
                   	<div style="margin: 20px;">
                        <div class="yuyuekanfang">
                        	<form>
                        		<textarea style="width:100%;height:200px;" id="content1" placeholder="   说的什么好呢。。。。" rows="" cols=""></textarea>
                        		 <button id="btn_fabiao" type="button" class="btn btn-default"> 发  表 </button>
                        		 <button id="btn_quxiao" type="button" class="btn btn-default" data-dismiss="modal"> 取  消 </button>
                        	</form>
                        </div>
                    </div>
                </div>
		...
		function initTable() {  
		    ...
		    $("#cusTable").bootstrapTable({  
		        ...
		        url: basePath+"/comment/all2", //获取数据的Servlet地址  
		  ...
		  function logoFormatter(value,row,index){
				return "<img src='"+basePath+"/upload/"+row.user.headicon+"' width='100' height='70' style='border-radius:300px;margin-right:20px;' alt='头像'>"+row.user.nickname;
		  }
		  $(function(){
      		$("#btn_look").bind("click",function(){
      			if($("#user_id").val()==""){
    				...
    				document.getElementById("asda").click(); 
    			}
      			
      		});
      		$("#btn_fabiao").bind("click",function(){
      			if($("#user_id").val()==""){
    				swal("提示信息", "您还未登录，请登录");
    				return false;
    			}else if($("#content1").val()==""){
    				swal("提示信息", "请输入您要评价的内容");
    				return false;
    			}else{
    				var url=basePath+"/comment/add";
        			$.post(url,{
            			"comment.buildings_id":$("#buildings_id").val(),
            			"comment.user_id":$("#user_id").val(),
            			"comment.content":$("#content1").val()
            			},
            			function(data){
            				swal("提示信息", data.msg);
            				document.getElementById("btn_quxiao").click();
