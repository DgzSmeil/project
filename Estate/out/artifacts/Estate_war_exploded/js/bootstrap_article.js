...
function initTable() {  
    ...
    $("#cusTable").bootstrapTable({  
        ...
        url: basePath+"/article/articlePager", //获取数据的Servlet地址  
        striped: true,  //表格显示条纹  
...
function logoFormatter(value,row,index){
  ...
  function formatDate(value) {
	...
	$("#bysearch").bind("click",function(){
		$('#myModal').modal('hide');
		window.location.href=basePath+"/article/search?article.title="+$(articleTitle).val(); 
		});
	...
	function dele(id){
		  swal({   
			  ...
			  function(isConfirm){   
				  if (isConfirm) {     
					  var url=basePath+"/article/active";
	        			$.post(url,{
	            			"article.id":id
	            			},
	            			function(data){
	            				swal(data.msg, "","success");
	            				$('#cusTable').bootstrapTable(('refresh'), basePath+"/article/articlePage");
	            			},
	            			"json"
	            		);
					  } else {     
...
swal({   
	  ...
	  function(isConfirm){   
		  if (isConfirm) {     
			  var url=basePath+"/article/inactive";
			$.post(url,{
    			"article.id":id
    			},
    			function(data){
