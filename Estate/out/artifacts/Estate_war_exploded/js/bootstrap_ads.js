...
function initTable() {  
    ...
    $("#cusTable").bootstrapTable({  
        ...
        url: basePath+"/ads/adsPager", //获取数据的Servlet地址  
        striped: true,  //表格显示条纹  
...
function logoFormatter(value,row,index){
  ...
  function logoFormatter7(value,row,index){
	  if(row.show_status=='Y'){
		  return "已显示";
	  }else if(row.show_status=='N'){
		  return "不显示";
	  }else if(row.show_status=='N')
	  {
		  return "";
  ...
  function logoFormatter8(value,row,index){
	  if(row.show_order==0){
		  return "从左到右";
	  }else if(row.show_order=1){
		  return "从上到下";
	  }else if(row.show_order=null)
	 {
		  return "";
	 }
  ...
  function formatDate(value) {
	...
	$("#bysearch").bind("click",function(){
		$('#myModal').modal('hide');
		window.location.href=basePath+"/ads/search?ads.title="+$(adsTitle).val(); 
		});
	...
	function dele(id){
		  swal({   
			  ...
			  function(isConfirm){   
				  if (isConfirm) {     
					  var url=basePath+"/ads/active";
	        			$.post(url,{
	            			"ads.id":id
	            			},
	            			function(data){
	            				swal(data.msg, "","success");
	            				$('#cusTable').bootstrapTable(('refresh'), basePath+"/ads/adsPage");
	            			},
	            			"json"
	            		);
					  } else {     
