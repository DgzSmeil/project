       ...
       <!-- about-us-area-start -->
                ...
                <div class="row">
                    <div class="col-md-6 col-sm-12 col-xs-12">
                         <div class="about-img wow fadeIn" data-wow-duration=".5s" data-wow-delay=".5s">
                            <img src="<%=path %>/upload/${house.logo}" alt="" />
                        </div>
                    </div>
                    <div class="col-md-6 col-sm-12 col-xs-12">
						...
						<div class="about-text wow fadeIn" data-wow-duration=".5s" data-wow-delay=".5s">
                            <h1>${house.name }</h1>
                            <h5>建筑面积：${house.area}㎡</h5>
                            <h5>参考均价：${house.unit_price }元/平方米</h5>
                            <h5>建造时间：${house.created_time }</h5>
                            <p>简介：${house.intro }</p>
                        </div>
                        <div class="product-action">
                            <button class="btn btn-default add-cart" title="预约看房" id="btn_click">预约看房</button>
                                    ...
                                    <div class="col-md-9">
                                        ...
                                        <center>
                                               <form>
                                               		...
                                               		<label>预约看房时间：</label>
                                               		<input type="date" id="app_arrive_time" class="center"><br/>
                                               		<label>&nbsp;&nbsp;预约员工&nbsp;&nbsp;&nbsp;：&nbsp;&nbsp;</label>
                                               		<select class="center" id="emp_id">
                                               			<s:iterator value="#request.employeelist" var="d">
                                               				<option value="${d.id }">${d.name }</option>
                                               			</s:iterator>
                                               		</select>
                                               		<br/>
                                               		<div class="product-action">
							                            <button class="btn btn-default add-cart" type="button" title="预约" id="btn_click1">预约</button>
							                            <button class="btn btn-default add-cart" type="button" class="close" data-dismiss="modal" id="btn_close">取消</button>
							                        </div>
                                               </form>
                                        </center>
                                        </div>
        ...
        var basePath = localObj.protocol + "//" + localObj.host + "/"+ contextPath;
        	$(function(){
        		...
        		$("#btn_click1").bind("click",function(){
        			if($("#user_id").val()==""){
        				swal("提示信息", "您还未登录，请登录");
        				return false;
        			}else{
        				if($("#app_arrive_time").val()==""){
            				swal("提示信息", "请选择看房时间");
            				return false;
            			}
            			var url=basePath+"/appointment/add";
            			$.post(url,{
                			"appointment.emp_id":$("#emp_id").val(),
                			"appointment.user_id":$("#user_id").val(),
                			"appointment.app_arrive_time":$("#app_arrive_time").val(),
                			"appointment.house_id":$("#house_id").val()
                			},
                			function(data){
                				swal("提示信息", data.msg);
