...
function initTable() {  
    ...
    $("#cusTable").bootstrapTable({  
        ...
        url: basePath+"/articleType/articleTypePager", //获取数据的Servlet地址  
        striped: true,  //表格显示条纹  
...
function logoFormatter(value,row,index){
		return "<img src='/Estate/upload/"+row.logo+"' style='width:180px;height:100px;'>";
  }
  ...
  function formatDate(value) {
		...
		$("#btn_look").click(function(){
			...
			if(data.length==1){
				window.location.href=basePath+"/articleType/articleTypeLook?articleType.id="+data[0].id; 
			}else{
				swal("提示信息", "请选中行", "info");
			}
		...
		$("#btn_add").click(function(){
			window.location.href=basePath+"/articleType/articleTypeAddPage"
		});
	
		$("#btn_edit").click(function(){
			var data=$("#cusTable").bootstrapTable("getSelections");
			if(data.length==1){
				window.location.href=basePath+"/articleType/articleTypeEditPage?articleType.id="+data[0].id; 
			}else{
				swal("提示信息", "请选中行", "info");
			}
		});
	...
	$("#bysearch").bind("click",function(){
		$('#myModal').modal('hide');
		window.location.href=basePath+"/articleType/search?articleType.name="+$(articleTypeName).val(); 
		});
	...
	function dele(id){
		  swal({   
			  ...
			  function(isConfirm){   
				  if (isConfirm) {     
					  var url=basePath+"/articleType/active";
	        			$.post(url,{
	            			"articleType.id":id
	            			},
	            			function(data){
	            				swal(data.msg, "","success");
	            				$('#cusTable').bootstrapTable(('refresh'), basePath+"/articleType/articleTypePage");
	            			},
	            			"json"
	            		);
					  } else {     
...
swal({   
	  ...
	  function(isConfirm){   
		  if (isConfirm) {     
			  var url=basePath+"/articleType/inactive";
			$.post(url,{
    			"articleType.id":id
    			},
    			function(data){
