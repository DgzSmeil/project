...
<div class="nav">
    <div class="nav_nr">
        <dl class="nav1">
            <dt><a href="<%=basePath %>courselistservlet?order=list" target="_blank"><img src='<%=basePath %>image/itlogo.png' alt=""></a></dt>
        </dl>
    </div>
</div>
...
<!--头部文件结束-->    <div class="zczh">
	<%
		StudentBean studetbean = (StudentBean)session.getAttribute("bean");
		MoneyBean moneybean = (MoneyBean)session.getAttribute("moneybean");
	%>
    <h2>
        <a onclick="window.history.go(-1);" href="javascript:void(0)">返回上一步</a>
    </h2>
    <div class="form-group-sjwc z_sjwc">
        <h3><span class="nadd">更新完成，欢迎提问！</span></h3>
        <h4> <a href="<%=basePath %>courselistservlet?order=list"><span id="mes">5</span>s 后跳到主页面</a></h4>
        <div class="z_dlW">
            <dl>
...
<script language="javascript" type="text/javascript">
    ...
    function fun() {
        if (num == 0) {
            var reback = "<%=basePath %>courselistservlet?order=list";            
