...
<body>
	<div class="pageheader">
		<h2><i class="fa fa-th-list"></i>广告管理 <span>所有广告</span></h2>
		     	...
		     	<thead> 
		          <tr>
		          	  <th data-radio="true" data-valign="middle"></th>                              
			       	  <th data-field="logo" data-valign="middle" data-formatter="logoFormatter">广告图片</th>            
		       		  <th data-field="title"  data-valign="middle">广告标题</th>  
		       		   <th data-field="show_status"  data-valign="middle" data-formatter="logoFormatter7">是否显示</th>  
		       		  <th data-field="show_order" data-valign="middle" data-formatter="logoFormatter8">显示顺序</th>
                ...
                <div class="modal-body" style="text-align:center;">
			   	<form class="form-horizontal form-bordered" method="post" action="<%=path %>/agency/agcedit" id="form1"  enctype="multipart/form-data">
						<div class="form-group" id="div2">
						  ...
						  <div class="col-sm-4">
							<input type="text" id="adsTitle" name="ads.title" data-placement="right" class="form-control tooltips">
						  </div>
...
<script src="<%=path %>/js/bootstrap_ads.js"></script>
...
$.contextMenu({
	 ...
	 add: {name: "新增", callback: function(key, opt){ 
		 window.location.href=basePath+"/ads/adsAddPage";
	 }},
	 housestyle: {name: "修改", callback: function(key, opt){ 
			var data=$("#cusTable").bootstrapTable("getSelections");
			if(data.length==1){
				window.location.href=basePath+"/ads/adsEditPage?ads.id="+data[0].id; 
			}else{
				swal("提示信息", "请选中行", "info");
			}
	 }},
	 detailinfo: {name: "详细资料", callback: function(key, opt){ 
		 var data=$("#cusTable").bootstrapTable("getSelections");
			if(data.length==1){
				window.location.href=basePath+"/ads/adsLook?ads.id="+data[0].id; 
			}else{
				swal("提示信息", "请选中行", "info");
			}
	 }},
